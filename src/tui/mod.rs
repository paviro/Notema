...
pub(crate) fn concise_error(error: &anyhow::Error) -> String {
    let tail: Vec<String> = error.chain().skip(1).map(|cause| cause.to_string()).collect();
...
mod tests {
    ...
    fn concise_error_drops_the_outer_path_prose_and_keeps_the_cause() {
        ...
        assert!(!concise.contains("Application"), "drops the path prose: {concise}");
        assert!(concise.contains("in `mood`"), "keeps the token context: {concise}");
        assert!(concise.contains("unrecognized color 'blorp'"), "keeps the cause: {concise}");
    }

    #[test]
    fn concise_error_falls_back_to_a_lone_error() {
        let err = anyhow::anyhow!("No such file or directory (os error 2)");
        assert_eq!(concise_error(&err), "No such file or directory (os error 2)");
    }
    ...
    fn concise_error_is_single_line_and_capped() {
        ...
        assert!(concise.chars().count() <= 121, "capped: {}", concise.chars().count());
