...
fn toast_style(theme: &Theme, variant: ToastVariant) -> Style {
    match variant {
...
pub(crate) fn toast_rects(app: &AppModel, area: Rect) -> Vec<Rect> {
    ...
    for toast in app.toasts.items() {
        ...
        let countdown_row = u16::from(!flat_chrome(&app.appearance.theme));
...
pub(crate) fn toast_at_point(app: &AppModel, area: Rect, col: u16, row: u16) -> Option<usize> {
...
fn draw_toast(
    ...
    let accent = toast_style(theme, variant);
    ...
    if flat_chrome(theme) {
        ...
        clear_surface(theme, frame, area, theme.raised_bg());
        if hovered {
        ...
        clear_surface(theme, frame, area, theme.content_bg());
        if hovered {
            frame.buffer_mut().set_style(area, theme.hover());
        }
        frame.render_widget(
    ...
    draw_countdown(theme, frame, area, accent, progress);
}
...
/// its bottom border.
fn draw_countdown(theme: &Theme, frame: &mut Frame<'_>, area: Rect, accent: Style, progress: f32) {
    ...
    let bottom_inset = if flat_chrome(theme) { 1 } else { 2 };
