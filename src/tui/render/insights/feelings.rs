...
pub(super) fn draw(
    ...
    if mood.mean.is_none() && mood.feelings.is_empty() {
        *scroll = 0;
        render_centered_notice(active_theme, frame, area, "No mood or feelings logged yet");
    ...
    if let Some(area) = sections[0] {
        let body = heading(active_theme, frame, area, "Balance");
        draw_balance(
            active_theme,
            frame,
            body,
            &mood.sentiment,
            &mood.sentiment_windows,
        );
    }

    if let Some(area) = sections[1] {
        draw_breakdowns(active_theme, frame, area, mood);
    }

    match sections[2] {
        Some(area) => {
            let body = heading(active_theme, frame, area, "Feelings");
...
fn draw_breakdowns(theme: &Theme, frame: &mut Frame<'_>, area: Rect, mood: &MoodAnalytics) {
    const GUTTER: u16 = 2;
    ...
    let body = heading(theme, frame, cells[0], "By year");
    draw_signed_columns(theme, frame, body, &year_values, &year_labels);
    let body = heading(theme, frame, cells[1], "By weekday");
    draw_signed_columns(theme, frame, body, &mood.by_weekday, &WEEKDAYS);
    let body = heading(theme, frame, cells[2], "By month");
    draw_signed_columns(theme, frame, body, &mood.by_month, &MONTHS);
}
