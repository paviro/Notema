...
//! the request is denied with no prompt. The `journal` binary embeds a signed
//! copy of the wrapping `.app`, extracts it, and runs this helper. See
//! `journal-storage`'s macOS device-location provider.
...
fn main() {
    eprintln!("journal-locate only runs on macOS");
    std::process::exit(1);
}
